...
# travels-data.js's spelling -> Natural Earth's admin-1 `name`, where they
# differ. The states come from ne_50m_admin_1_states_provinces: the 110m set
# has no admin-1 layer at all, and 10m is 100 MB for outlines drawn 46 px wide.
STATE_ALIAS = {
...
def travels_source():
    ...
    # Every state or province a trip names, from the `states` table and from
    # the `where` / `crossed` lists on the trips themselves. Read the same way
    # as the countries above so this tool still keeps no list of its own.
    st = src[src.index('states: {'):]
    st = st[:st.index('\n  },')]
    states = [unesc(m) for m in re.findall(r"^\s*'([^']+)':\s*\{", st, re.M)]
    for field in ('where', 'crossed'):
        for block in re.findall(field + r': \[(.*?)\],\n', src, re.S):
            for m in re.findall(r"'([^']+)'", block):
                n = unesc(m)
                if n not in states:
                    states.append(n)
    return countries, balkans, states
...
def main():
    countries, balkans, states = travels_source()
    ...
    # Same pipeline, a different Natural Earth layer. They go in the SAME dict
    # as the countries, keyed by their own name, because travels.js looks a
    # shape up by name and does not care which layer drew it. A state and a
    # country can never collide here: no US state is spelled like a country.
    if states:
        a1 = fetch('ne_50m_admin_1_states_provinces.geojson')['features']
        idx = {}
        for f in a1:
            pr = f['properties']
            for key in ('name', 'name_en', 'gn_name'):
                v = pr.get(key)
                if v and v not in idx:
                    idx[v] = f
        for name in states:
            ne = STATE_ALIAS.get(name, name)
            f = idx.get(ne)
            if not f:
                missing.append(name)
                continue
            d, w, h = outline(rings_of(f))
            out[name] = (d, w, h)
            print('  %-22s %5d chars   %3d x %3d  (state)' % (name, len(d), round(w), round(h)))
    ...
    body = ['''/* travels-shapes.js: one simplified outline per country, state and
   province on /travels/.

   GENERATED by tools/shapes.py from Natural Earth: countries from 110m (50m
   for Malta and Singapore, which 110m is too coarse to contain), states and
   provinces from 50m admin-1, which is the coarsest set that has them at all.
   Do not edit by hand; add a country to `world`, or a state to `states` or to
   a trip's `where` / `crossed`, in travels-data.js and re-run the tool.

   Countries and states share this one dictionary and are looked up by name
   alone, which is safe because no state here is spelled like a country.
